...
import 'package:medico_app/providers/log/log_notifikasi_provider.dart';
...
class IndexLogNotifikasi extends StatefulWidget {
  const IndexLogNotifikasi({Key? key}) : super(key: key);

  @override
  _IndexLogNotifikasiState createState() => _IndexLogNotifikasiState();
}

class _IndexLogNotifikasiState extends State<IndexLogNotifikasi> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          context.refresh(logNotifikasiProvider);
        },
        child: Column(
          children: [
            Expanded(
              child: Consumer(
                builder: (context, watch, child) {
                  final reservationData = watch(logNotifikasiProvider);
  ...
  void _scrollListener() async {
    if (controller.position.extentAfter == 0) {
      ...
      await context.read(logNotifikasiProvider.notifier).nextPage();

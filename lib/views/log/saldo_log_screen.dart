...
import 'package:medico_app/providers/log/log_saldo_provider.dart';
...
class IndexLogSaldo extends StatefulWidget {
  const IndexLogSaldo({Key? key}) : super(key: key);

  @override
  _IndexLogSaldoState createState() => _IndexLogSaldoState();
}

class _IndexLogSaldoState extends State<IndexLogSaldo> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          context.refresh(logSaldoProvider).getLogSaldo();
        },
        child: Column(
          children: [
            Expanded(
              child: Consumer(
                builder: (context, watch, child) {
                  final saldoData = watch(logSaldoProvider);
                  return saldoData.when(
                    data: (data) {
                      if (data.length == 0) {
                        return Container(
                          child: Center(
  ...
  void _scrollListener() async {
    if (controller.position.extentAfter == 0) {
      ...
      await context.read(logSaldoProvider.notifier).nextPage();
